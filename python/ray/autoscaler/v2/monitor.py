...
from ray.autoscaler.v2.instance_manager.config import AutoscalerConfig
...
def run(
    ...
    node_provider = NodeProviderAdapter(
        ...
        instance_config_provider=AutoscalerConfig(node_configs=autoscaling_config),
    )
        ...
        subscribers.append(
            ThreadedRayInstaller(
                ...
                ray_installer=RayInstaller(
                    provider=node_provider_v1,
                    config=AutoscalerConfig(node_configs=autoscaling_config),
                ),
            )
        )
